...
def compile_model(non_2D_shape, lastlayerdimension, learn_rate, loss_func):
    ...
    a = Conv2D(
        2 * filter_size, kernel_size=(1, 1), activation=activation_comb, padding=padding
    )(a)
    a = Flatten()(a)
    ...
    j = Conv2D(
        filter_size, kernel_size=kernel_size, activation=activation, padding=padding
    )(input_layer_12)
    ...
    j = Conv2D(
        2 * filter_size, kernel_size=(1, 1), activation=activation, padding=padding
    )(j)
    j = Flatten()(j)
    ...
    # Create the model
    model = Model(
        inputs=[input_layer_1, input_layer_12, input_layer_6], outputs=gmm_params
    )

    # compile the model with a learning rate and custom loss function
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learn_rate, clipvalue=1.0),
        loss=loss_func,
        metrics=["accuracy"],
    )
