...
for agn_file in agn_files:
	...
	lrg_file = agn_file.replace('_agn.npy', '_LRG.npy')
	if not os.path.exists(cosmos_file):
		print(f"Warning: Cosmos file {cosmos_file} not found for {agn_file}")
		continue
	if not os.path.exists(lrg_file):
                print(f"Warning: LRG file {lrg_file} not found for {agn_file}")
                continue
	...
	try:
		...
		lrg_data = np.load(lrg_file, allow_pickle=True)
	except Exception as e:
		print(f"Error loading {agn_file} or {cosmos_file} or {lrg_file}: {e}")
		continue
	...
	if isinstance(lrg_data, np.ndarray) and lrg_data.shape == ():
                lrg_data = lrg_data.item()


	# Now handle based on type
	if isinstance(agn_data, np.ndarray) and isinstance(cosmos_data, np.ndarray) and isinstance(lrg_data, np.ndarray):
		...
		if agn_data.shape[0] != cosmos_data.shape[0] or agn_data.shape[2:] != cosmos_data.shape[2:]:
			...
			continue
		if agn_data.shape[0] != lrg_data.shape[0] or agn_data.shape[2:] != lrg_data.shape[2:]:
			print(f"Shape mismatch between {agn_file} and {lrg_file}, skipping.")
			continue

		combined_data = np.concatenate([agn_data, cosmos_data, lrg_data], axis=1)
		print(f'{agn_file}: {combined_data.shape}')

	elif isinstance(agn_data, dict) and isinstance(cosmos_data, dict) and isinstance(lrg_data, dict):
		...
		for key in agn_data:
			...
			if key not in lrg_data:
                                print(f"Key {key} missing in lrg data, skipping.")
                                continue


			# Concatenate values under this key
			combined_data[key] = np.concatenate([agn_data[key], cosmos_data[key], lrg_data[key]], axis=0)
	...
	np.save(data_dir +"3pt/" + base_name, combined_data)
